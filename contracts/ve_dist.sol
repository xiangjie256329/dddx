...
contract ve_dist {
    ...
    uint public immutable start_time;//开始时间
    ...
    address public immutable voting_escrow;//NEW_ADD 移除immutable
    address public immutable token;//NEW_ADD 移除immutable
    ...
    function _checkpoint_token() internal { //根据周时间戳更新一个token的每周收益
    ...
    function _claimable(uint _tokenId, address ve, uint _last_token_time) internal view returns (uint) {
        ...
        for (uint i = 0; i < 50; i++) {
            ...
            if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
                ...
                uint balance_of = Math.max(uint(int256(old_user_point.bias - dt * old_user_point.slope)), 0);//剩余锁仓占比
                if (balance_of == 0 && user_epoch > max_user_epoch) break;
                if (balance_of > 0) {
