...
contract BaseV1Minter {
    ...
    uint public weekly = 2900000e18;//NEW_ADD 20000000e18
    ...
    event Initialize(address indexed initializer, uint active_period);//NEW_ADD
...
//        active_period = (block.timestamp + week) / week * week;
        active_period = 1648684800;  // 2022.3.31 NEW_ADD

        emit Initialize(initializer, active_period);//NEW_ADD
    }
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + week && initializer == address(0)) { // only trigger if new week 如果是新的一周,并且已经初始化
            ...
            uint256 weeklyEmission = weekly_emission();//NEW_ADD weekly = weekly_emission();//更新weekly 慢慢减少

            uint _growth = calculate_growth(weeklyEmission);//NEW_ADD weekly
            uint _required = _growth + weeklyEmission;//NEW_ADD weekly
            ...
            _token.approve(address(_voter), weeklyEmission);//NEW_ADD weekly
            _voter.notifyRewardAmount(weeklyEmission);//NEW_ADD weekly

            emit Mint(msg.sender, weeklyEmission, circulating_supply(), circulating_emission());//NEW_ADD weekly

            //NEW_ADD
            if(calculate_emission() > circulating_emission()){ // Inflation is falling by 2 per cent at a time  每周缩减2%
