...
interface IERC721Receiver {
    ...
    *
    * It must return its DDDX selector to confirm the token transfer.
    * If any other value is returned or the interface is not implemented by the recipient, the transfer will be reverted.
    *
    * The selector can be obtained in DDDX with `IERC721.onERC721Received.selector`.
    */
...
contract ve is IERC721, IERC721Metadata {
    ...
    function _checkpoint(
        ...
        if (_tokenId != 0) {
            ...
            //NEW_ADD
            if(old_locked.end > block.timestamp) slope_changes[old_locked.end] += u_old.slope;//如果旧的锁仓还没结束
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) internal pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "dddx locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
