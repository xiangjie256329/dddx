...
describe("oracles", function () {
  ...
  it("deploy base coins", async function () {//部署代币
  ...
  it("deploy BaseV1Factory and test pair length", async function () {//部署factory
  ...
  it("deploy pair via BaseV1Factory owner", async function () {//添加流动性
  ...
  it("mint & burn tokens for pair mim-ust", async function () {
    ...
    console.log("mint")
    let lp = await pair.mint(owner2.address);//mint都是在一个函数中执行的,不至少用别人的
    //console.log("lp:",lp);
