...
describe("washtrade", function () {
  ...
  it("ve merge", async function () {
    ...
    console.log("balanceOfNFT 1",await ve.balanceOfNFT(1))
    console.log("balanceOfNFT 2",await ve.balanceOfNFT(2))
    await ve.merge(2, 1);
  ...
  it("deploy pair via BaseV1Factory owner", async function () {
    ...
    await router.addLiquidity(mim.address, ust.address, true, mim_1, ust_1, 0, 0, owner.address, Date.now());//稳定池
    await mim.approve(router.address, mim_1);
    await ust.approve(router.address, ust_1);
    await router.addLiquidity(mim.address, ust.address, false, mim_1, ust_1, 0, 0, owner.address, Date.now());//不稳定池
    await mim.approve(router.address, mim_1);
    ...
    console.log("pair0 addr:",await factory.allPairs(0));
    console.log("pair1 addr:",await factory.allPairs(1));
    console.log("pair2 addr:",await factory.allPairs(2));
  });
  ...
  it("deploy BaseV1Voter", async function () {
    ...
    baseV1Voter = await BaseV1Voter.deploy(ve.address, factory.address, gauges_factory.address, bribe_factory.address);
    await baseV1Voter.deployed();
    //这里先加入了一些白名单币种和minter地址
    await baseV1Voter.initialize([ust.address, mim.address, dai.address, ve_underlying.address],owner.address);

    expect(await baseV1Voter.length()).to.equal(0);
  });

  it("deploy BaseV1Factory gauge", async function () {
    const pair_1000 = ethers.BigNumber.from("1000000000");

    await ve_underlying.approve(baseV1Voter.address, ethers.BigNumber.from("500000000000000000000000"));//500000
    await baseV1Voter.createGauge(pair3.address);//创建一个奖池
    expect(await baseV1Voter.gauges(pair.address)).to.not.equal(0x0000000000000000000000000000000000000000);

    const gauge_address3 = await baseV1Voter.gauges(pair3.address);
    const bribe_address3 = await baseV1Voter.bribes(gauge_address3);
    ...
    const total = await pair3.balanceOf(owner.address);//把owner的lp存进奖池
    console.log("total:",total/unit);
    let derivedBalance = await gauge3.derivedBalance(owner.address);
    console.log("derivedBalance1:",derivedBalance/unit);

    await pair3.approve(gauge3.address, total);//min和dai的lp总数
    await gauge3.deposit(total, 0);
    expect(await gauge3.totalSupply()).to.equal(total);
    derivedBalance = await gauge3.derivedBalance(owner.address);
    console.log("derivedBalance2:",derivedBalance/unit);
    expect(await gauge3.earned(ve.address, owner.address)).to.equal(0);

    let balanceAmount = await gauge3.balanceOf(owner.address);
    console.log("balanceAmount:",balanceAmount/unit);
  });
  ...
  it("BaseV1Router01 pair3 getAmountsOut & swapExactTokensForTokens", async function () {
    ...
    console.log("bribe3 balance111:",await mim.balanceOf(bribe3.address));
  });

  it("gauge reset", async function () {
    await ve.setVoter(baseV1Voter.address);
    await baseV1Voter.reset(1);
  });

  it("gauge poke self", async function () {
    await baseV1Voter.poke(1);
  });

  it("gauge vote & bribe balanceOf", async function () {
    let totalWeight = await baseV1Voter.totalWeight();
    console.log("before vote,total weight:",totalWeight);
    await baseV1Voter.vote(1, [pair3.address, pair2.address], [5000,5000]);//投2个池
    totalWeight = await baseV1Voter.totalWeight();
    console.log("total weight:",totalWeight/unit);
    expect(await baseV1Voter.totalWeight()).to.not.equal(0);
    expect(await bribe3.balanceOf(1)).to.not.equal(0);
  });

  it("bribe claim rewards", async function () {//这里并没有收益
    let reward = await bribe3.getReward(1, [mim.address, dai.address]);
  ...
  it("distribute and claim fees", async function () {
    ...
    console.log("bribe3 balance112:",await mim.balanceOf(bribe3.address));
    await baseV1Voter.distributeFees([gauge3.address])//只有做了这个操作,才可以getReward
    console.log("bribe3 balance113:",await mim.balanceOf(bribe3.address));
  });

  it("bribe claim rewards", async function () {
    console.log(await bribe3.earned(mim.address, 1));
    console.log(await mim.balanceOf(owner.address)/unit);
    console.log(await mim.balanceOf(bribe3.address)/unit);
    await bribe3.batchRewardPerToken(mim.address, 200);//将几个检查点的数据汇总
    await bribe3.batchRewardPerToken(dai.address, 200);
    await bribe3.getReward(1, [mim.address, dai.address]);
    let earned = await bribe3.earned(mim.address, 1);
    let earned2 = await bribe3.earned(dai.address, 1);
    console.log("before increase:",earned/unit);
    console.log("before increase,dai eared:",earned2/unit);
    await network.provider.send("evm_increaseTime", [691200])
    await network.provider.send("evm_mine")
    earned = await bribe3.earned(mim.address, 1);
    earned2 = await bribe3.earned(dai.address, 1);
    console.log("earned:",earned/unit);
    console.log("dai earned:",earned2/unit);
    console.log("owner balance:",await mim.balanceOf(owner.address));
    console.log("bribe3 balance:",await mim.balanceOf(bribe3.address));
    await bribe3.getReward(1, [mim.address, dai.address]);
    console.log("mim eared:",await bribe3.earned(mim.address, 1));
    console.log("owner balance:",await mim.balanceOf(owner.address));
    console.log("bribe3 balance:",await mim.balanceOf(bribe3.address));
  });
