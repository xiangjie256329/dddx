...
describe("imbalance", function () {
  ...
  it("deploy base coins", async function () {
    ...
    await mim.mint(owner.address, ethers.BigNumber.from("1000000000000000000000000000000"));//1000000000000
  ...
  it("ve merge", async function () {
    ...
    let balanceOfNFT1 = await ve.balanceOfNFT(1);
    let balanceOfNFT2 = await ve.balanceOfNFT(1);
    console.log("balanceOfNFT1:",balanceOfNFT1/unit)
    console.log("balanceOfNFT2:",balanceOfNFT2/unit)
    await ve.merge(2, 1);
    balanceOfNFT1 = await ve.balanceOfNFT(1);
    console.log("balanceOfNFT1:",balanceOfNFT1/unit)
  ...
  it("mint & burn tokens for pair mim-ust", async function () {
    ...
    let balancePair3 = await pair3.balanceOf(owner.address);
    console.log("balancePair3:",balancePair3/unit);
  ...
  it("BaseV1Router01 addLiquidity", async function () {
    ...
    let balancePair3 = await pair3.balanceOf(owner.address);
    console.log("balancePair31:",balancePair3/unit);
    await ust.approve(router.address, ust_1000);
    await mim.approve(router.address, mim_1000);
    ...
    balancePair3 = await pair3.balanceOf(owner.address);//pair3主要是mim和dai
    console.log("balancePair32:",balancePair3/unit);
  });
  ...
  it("deploy BaseV1Factory gauge", async function () {
    ...
    await gauge_factory.createGauge(pair3.address);//拿mim,dai,稳定池创建奖池
    ...
    const bribe_address3 = await gauge_factory.bribes(gauge_address3);//获取贿赂池地址
  ...
  it("BaseV1Router01 pair3 getAmountsOut & swapExactTokensForTokens", async function () {
    ...
    const route = {from:mim.address, to:dai.address, stable:true}//路由
    ...
    console.log("before transfer,mb:%d,db:%d",mb/unit,db/unit);
    ...
    const ma = await mim.balanceOf(owner.address);
    const da = await dai.balanceOf(owner.address);
    console.log("after transfer,mb:%d,db:%d",ma/unit,da/unit);

    const net_after = ma.add(da);
    const net_before = db.add(mb)

    expect(net_before).to.be.above(net_after);
  });
  ...
  it("BaseV1Router01 pair3 getAmountsOut & swapExactTokensForTokens", async function () {
    ...
    const mim_100000000 = ethers.BigNumber.from("10000000000000000000000000000");
    const dai_100000000 = ethers.BigNumber.from("10000000000000000000000000000");
    await dai.approve(router.address, dai_100000000);
    await mim.approve(router.address, mim_100000000);
    const pair_before = await pair3.balanceOf(owner.address);
    await router.addLiquidity(mim.address, dai.address, true, mim_100000000, dai_100000000, 0, 0, owner.address, Date.now());
    ...
    await pair3.approve(router.address, lp_balance);
    await router.removeLiquidity(mim.address, dai.address, true, lp_balance, 0, 0, owner.address, Date.now());
